...
import { deleteGrammarCheckAction } from "@/lib/grammar/actions";
import { routes } from "@/lib/config/routes";

/** Deletes a check and the text stored with it. Confirms first. */
export function DeleteCheckButton({ checkId }: { checkId: string }) {
  ...
  function remove() {
    startTransition(async () => {
      const result = await deleteGrammarCheckAction(checkId);
      if (!result.ok) {
        setError(result.error);
        setConfirming(false);
        return;
      }
      router.push(routes.grammar);
    });
  }
